...
import CIS_HAS_ASMR from '../model/CIS_HAS_ASMRModel.mjs';
...
const schema = new mongoose.Schema({
    ...
    valueASMR : {type:String,required:true},
    libelleASMR: {type:String,required:true}
})

const MongoCIS_HAS_ASMR = new mongoose.model('CIS_HAS_ASMRCollection',schema)
const CIS_HAS_ASMRDAO = {

    // Remplissage de la base de données avec la data provenant du fichier source
    populate : async()=> {
        await MongoCIS_HAS_ASMR.deleteMany({})
     
        try{
         
            //obligatoire, pour gérer les environnemments linux et Windows.
            const filePath = path.join('data', 'tables-utf8', 'CIS_HAS_ASMR_bdpm.txt');
            ...
            for (let i=0;i<DATA.length;i++){
                ...
                const elementClear= new CIS_HAS_ASMR({
                    codeCIS: DATA[i][0],
                    codeDossierHAS: DATA[i][1],
                    motifEval: DATA[i][2],
                    dateAvisCommisTranspa: DAO_ABSTRACT.yearMonthDayStringToDate(DATA[i][3]),
                    valueASMR: DATA[i][4],
                    libelleASMR: DAO_ABSTRACT.libelleTreatement(DATA[i][5])
                })
                
                await MongoCIS_HAS_ASMR.create(elementClear)
            };
    ...
    findAll :async (page,size)=> {
        const result = await MongoCIS_HAS_ASMR.aggregate([
            DAO_ABSTRACT.createPagination(page, size)
        ]);
        const metadata = result[0]?.metadata?.[0] || { totalCount: 0 };
        const data = result[0]?.data || [];

        return DAO_ABSTRACT.jsonToReturn(page, size, metadata, data, CIS_HAS_ASMR)
    },
    ...
    findByCodeCIS : async(codecis_API) => {
        ...
        const result = await MongoCIS_HAS_ASMR.aggregate(pipeline)
        const metadata = result[0]?.metadata?.[0] || { totalCount: 0 };
        const data = result[0]?.data || [];
        return DAO_ABSTRACT.jsonToReturn(1, 99, metadata, data, CIS_HAS_ASMR)
    }
}



export {CIS_HAS_ASMRDAO, MongoCIS_HAS_ASMR}